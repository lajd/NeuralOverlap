...
function plot_knn_distances(distance_matrix::Array, id_seq_data_map::Dict; plot_save_path::String=".", num_samples::Int64=10, identifier::String="", sampled_top_k_nns::Int64=100)
    for i in 1:num_samples
        ...
        fig = plot(scatter(1:length(distances), distances), title="Random KNN distances", xlabel="KNN index", ylabel="Distance")
        save_dir = joinpath(plot_save_path, "random_knn_distances")
        mkpath(save_dir)
        savefig(fig, joinpath(save_dir, string(identifier, "_", string(i), ".png")))
    end
end
...
function plot_triplet_batch_distances(tripletBatchDict::Dict, plot_save_path::String=".")
    ...
    fig = plot(scatter(1:length(Dpos), [Dpos, Dneg, Dposneg], label=["Dpos" "Dneg" "Dposneg"], title="Triplet batch distances"),  xlabel="Triplet index", ylabel="Pair distance")
