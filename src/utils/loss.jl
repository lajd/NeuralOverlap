...
function triplet_loss(args, Xacr::ArrayType, Xpos::ArrayType,
    ...
    posEmbedDist = embedding_distance(Embacr, Embpos, args.DISTANCE_METHOD, dims=1)  # 1D dist vector of size bsize
    negEmbedDist =  embedding_distance(Embacr, Embneg, args.DISTANCE_METHOD, dims=1)
    PosNegEmbedDist =  embedding_distance(Embpos, Embneg, args.DISTANCE_METHOD, dims=1)
